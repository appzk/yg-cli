  ...
  if (channelServer !== undefined && lodash.startsWith(channelServer, 'http')) {
    ...
    if (!!rcApiPrefix) {
      if (lodash.indexOf(rcApiPrefix.envs, channelServer) !== -1) {
        // ygego 单独处理www
        const tempChannelName = channelName;
...
exports.updateSettingUrls = async function(params) {
  ...
  for (let i = 0; i < envs.length; i += 1) {
    ...
    httpUrls[curr_env] = {
      ...
      exYgego:
        curr_env !== 'prod' ? `http://ex.ygego.beta2` : 'https://ex.ygyg.cn',
  ...
  this.writeJson(someFile, { httpUrls: httpUrls });
};

exports.writeJson = async function(someFile, params) {
  //现将json文件读出来
  const existFile = await this.isFileSync(someFile);
  if (!existFile) {
    fs.appendFile(someFile, '{}', 'utf8', function(err) {
      if (err) {
        this.logStep(err);
      } else {
  ...
  const readFile = util.promisify(fs.readFile);
  const writeFile = util.promisify(fs.writeFile);
  let setting = await readFile(someFile).then(
    data => {
  ...
  if (setting) {
    ...
    await writeFile(someFile, result).then(
      // data => {
      //   this.logStep(`success write setting ${result}`);
      // },
      err => {
        console.error(err);
      }
